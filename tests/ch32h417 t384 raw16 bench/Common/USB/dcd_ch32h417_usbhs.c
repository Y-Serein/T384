...
static void reset_endpoint_state(void)
{
    ...
    USBHSD->UEP_TX_BURST = 0u;
    USBHSD->UEP_TX_BURST_MODE = 0u;
    USBHSD->UEP_RX_BURST = 0u;
    USBHSD->UEP_RX_RES_MODE = 0u;
...
static void prepare_next_packet(uint8_t ep, tusb_dir_t dir)
{
    ...
    if (dir == TUSB_DIR_IN) {
        ...
        const uint8_t ep_mask = (uint8_t)(1u << ep);
        uint16_t count = TU_MIN(remaining, xfer->max_packet);

        if (xfer->burst) {
            /* In burst mode UEPn_TX_LEN is the whole contiguous transfer and
             * UEPn_MAX_LEN is the per-packet DMA stride. Suppress the
             * controller's implicit ZLP for normal transfers; TinyUSB queues
             * an explicit zero-length transfer when the protocol needs one. */
            count = remaining;
            if (count == 0u) {
                USBHSD->UEP_TX_BURST_MODE &= (uint8_t)~ep_mask;
            } else {
                USBHSD->UEP_TX_BURST_MODE |= ep_mask;
            }
        }
...
bool dcd_edpt_open(uint8_t rhport, tusb_desc_endpoint_t const *desc_ep)
{
    ...
    if (dir == TUSB_DIR_IN) {
        ...
        if (xfer->burst) {
            USBHSD->UEP_TX_BURST |= (uint8_t)(1u << ep);
            USBHSD->UEP_TX_BURST_MODE |= (uint8_t)(1u << ep);
        }
        EP_TX_LEN(ep) = 0u;
        EP_TX_CTRL(ep) = USBHS_UEP_T_RES_NAK | USBHS_UEP_T_TOG_DATA0;
    } else {
