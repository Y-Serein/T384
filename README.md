...
- 2026-09-17当前主线：真实MINI2已按B方案由V5F采集、V3F网络输出；双帧v2两核启动和五段内存访问自检通过，Windows60秒1174完整帧/19.567FPS/4.328MB/s，跳号626，逆序和半帧0，仍未达到用户≥25完整FPS目标。单帧HTTP关闭回调/静态窗口修复曾通过60秒和3次重连；双帧并发诊断抓流仍有读超时，TCP对齐读取优化已目标编译但未下载验证。当前状态以HANDOFF顶部为准；下面v6/v7为迁移前历史。

- 2026-09-17最新实机证据：v6已烧录且用户已整机断电重连，55帧均288行/221184B，逐行计数缺口不再出现；但10槽满/无空槽55次、发布0，随后源计数停止。接收器重启71→209仍无新帧，CR0=67/CR1=1；3秒只读单流无分块，NCM无丢弃/背压。停顿后的真实模组状态尚未知，启动时的回读不能证明此时仍在输出。
- 当前源码为`mini2-dvp-y16-picture-v7`：384故障时有界回读数字状态及模式，模式一致且回执有效才重发现有易失0x46 DVP启流；后续回读确认才重新启用MCU接收，写入间隔至少4秒，等待应用期间不重复写。读表期间禁止恢复，正常连续输入不查询/写入，256保留原路径；不写模式/探测器/保存/标定命令，不改USB/帧流。v7未上板验证，不能宣称384出图或标定完成。
- v6的384固定长度DMA（8行/6144B事件、10槽加2个暂存块）保留；11:51:04 v6 map栈前余量35160B，但早于v7源码，不能作为v7放行依据。采集数据预算仍73728B，不缓存整帧。
- 历史v3/v4：0x86查询已依据原厂SDK统一为3字节，v5补有界状态确认。384探测器60 Hz/DVP输出30 FPS仍独立配置，256为50/50；不承诺384全量RAW16 60 FPS吞吐。
- 2026-09-16：用户确认384已到货；正式配置默认切到 `T384_RAW16_PROFILE=384u`，保留256编译档。主机工具补齐384完整帧与黑体采集适配；384实际PN/FW、出图、原厂表与标定保存/回读/应用仍未上板验证，当前旧map不能证明新配置。
...
- 正式 `firmware/` 已建立分块链路；历史DMA等效源使用12×6144 B ring，再经
...
- 2026-09-17：设备v5已在 `192.168.17.1/24`提供HTTP诊断；源码DHCP池 `.17.2–.20`，Router/DNS均指向设备，本地DNS解析 `ir.sipeed.com`，常见Captive Portal探测跳转到IP入口。HTTP实测不替代DNS/手机兼容性验收；末次只读访问17.1超时。私有分块帧流保留，尚非完整统一协议；网段持久化/恢复及安全OTA未接入，测试快照不改。
...
`bash tools/check_raw16_bench.sh`。正式双核固件由用户在 Windows/MounRiver Studio 2.5.0
中先Build V3F，再Build V5F；本轮先核对两核map/HEX与合并产物，不下载旧单核HEX。正式入口为 `firmware/T384-RAW16-BENCH.wvsln`。
...
正式工程已切到B方案双核，已验证测试快照仍只构建V3F；两者均关闭 `Erase All` 与 `Clear CodeFlash`。双核内存和首次启动尚待目标构建/实板验证；
